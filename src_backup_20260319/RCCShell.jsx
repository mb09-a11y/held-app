  ...
  useEffect(() => {
    ...
    async function handleResume() {
      ...
      try {
        ...
        // ── Step 4: (key already incremented immediately on visibility change) ──
        // A second increment here after auth/data sync ensures modules that
        // loaded stale data from cache get refreshed with the confirmed server state.
    ...
    //
    // KEY FIX: increment checkinRefreshKey IMMEDIATELY on tab-back, before
    // any async work. Child modules (HeldHome, HeldSleepShell, Milestones, etc.)
    // all gate their fetches on activeFamily?.id — if we wait until after the
    // GoTrue lock resolves to signal them, their useEffect deps haven't changed
    // so they never re-trigger. Firing the key increment synchronously means
    // they re-fetch immediately using whatever data is already in context
    // (seeded from localStorage cache), giving instant data without waiting
    // for the auth roundtrip to complete.
    function scheduleResume() {
      clearTimeout(timer);
      // Immediate: unblock child module fetches right now
      if (document.visibilityState === "visible") {
        setCheckinRefreshKey(k => k + 1);
      }
      // Delayed: auth validation + family/children re-sync in background
